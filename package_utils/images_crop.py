...
ROOT = '/home/teaching/g_37/datasets/ff++'
#SAVE_DIR = f'{ROOT}/Celeb-DFv2/c23'
#SAVE_DIR = f'{ROOT}/FaceForensics++/c23'
SAVE_DIR = f'{ROOT}/DeepFakeDetection/c23'
        ...
        try:
            ...
            try:
                ...
                for face_idx in range(len(faces)):
                    x0,y0,x1,y1 = map(int, faces[face_idx]['bbox'])
...
if __name__=='__main__':
    ...
    parser.add_argument('-d', dest='dataset', 
                              choices=['FaceShifter','Face2Face','Deepfakes','FaceSwap','NeuralTextures',\
                                       'Original','Celeb-real','Celeb-synthesis','YouTube-real','DFDC','DFDCP','method_A','method_B','original_videos', 'DeepFakeDetection_original', 'DeepFakeDetection'])
    ...
    if args.dataset=='Original':
        ...
        dataset_path='{}/original_sequences/actors/{}/'.format(ROOT, args.comp)
    ...
    labels = []
    if args.dataset in ['Original','DeepFakeDetection','FaceShifter','Face2Face','Deepfakes','FaceSwap','NeuralTextures', 'DeepFakeDetection_original']:
        ...
        with open(f'{ROOT}/original_sequences/youtube/c23/{args.task}.json') as f:
            vid_ids = json.load(f)
    elif args.dataset in ['Celeb-real', 'Celeb-synthesis', 'YouTube-real']:
        if 'v1' in SAVE_DIR:
    ...
    for i in range(len(vid_ids)):    
        if args.dataset == 'Original':   
            file_list += vid_ids[i]
        elif args.dataset in ['Face2Face','Deepfakes','FaceSwap','NeuralTextures', 'DeepFakeDetection']:
